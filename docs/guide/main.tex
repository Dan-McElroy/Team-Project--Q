...
There are a few known issues with the current build of the system.
If you should find any bugs not listed below, please let us know
either in person or by e-mail (\texttt{TeamProjectQ@gmail.com}).

\begin{itemize}
\item{On both the Target Area Selection and Primer Design screens
    (referred to in sections \ref{sec:targetAreaSelection} and
    \ref{sec:primerDesign}) the ``Both Strands'' tab's numbers on the
    left of the screen do not match the lines of the sequence.}
\item{On the Primer Design screen's (section \ref{sec:primerDesign})
    ``Both Strands'' tab, the highlighting sometimes does not
