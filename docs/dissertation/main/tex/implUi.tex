...
entirety and see the ``Further Reading'' text pane for useful links.
...
assessment and specifying that the user must use a particular sequence
with a target from base 50 to base 506 for example, allowing greater
versatility in the manner in which the program is used.
...
teach.
Another issue was the ambiguity of whether the reverse primer has been
...
rules without the need to check it with the dialogue windows every
time and the reverse button has been replaced to reduce ambiguity and
emphasise the sequence direction, adding to users' understanding.
